...
const DeliveryDashboard: React.FC<DeliveryDashboardProps> = ({ token }) => {
  ...
  const statusOptions = [
    { value: 'OW', label: t('client_dashboard.status_ow') },
  ...
  const fetchOrders = async () => {
    try {
      ...
      setError(t('delivery_dashboard.error') + ' ' + (err.response?.data || err.message));
    }
  };
  ...
  const fetchOrderDetails = async (orderId: number) => {
    try {
      ...
      setError(t('delivery_dashboard.error_details') + ' ' + (err.response?.data?.error || err.message));
    }
  };
  ...
  const updateOrderStatus = async (orderId: number) => {
    ...
    try {
      ...
      setOrders(prevOrders =>
        prevOrders.map(order =>
          order.id === orderId ? { ...order, ...response.data, statusHistory: [...(order.statusHistory || []), { id: Date.now(), orderId, status, updatedBy: 'delivery', updatedAt: new Date().toISOString() }] } : order
        )
      );
      setPendingStatus(prev => ({ ...prev, [orderId]: '' }));
      setError('');
    } catch (err: any) {
      setError(t('delivery_dashboard.error_update_status') + ' ' + (err.response?.data || err.message));
    }
  };
  ...
  useEffect(() => {
    ...
    client.connect(
      ...
      (error: unknown) => {
        ...
        setError(t('delivery_dashboard.websocket_error'));
  ...
  return (
    <div className="container mx-auto px-6 py-8">
      <h2 className="text-4xl font-semibold text-gray-800 mb-6 animate-fade-in">{t('delivery_dashboard.title')}</h2>
      {error && <p className="text-red-500 mb-6 font-medium bg-red-50 p-4 rounded-lg animate-fade-in">{error}</p>}
      {orders.length === 0 ? (
        <p className="text-gray-600 text-lg animate-fade-in">{t('delivery_dashboard.no_orders')}</p>
      ) : (
              ...
              <div className="flex justify-between items-center">
                <div>
                  <p className="text-gray-700 font-medium">
                    {t('delivery_dashboard.order_id')}: <span className="text-blue-600">{order.id}</span>
                  </p>
                  <p className="text-gray-600">
                    {t('delivery_dashboard.order_status')}: <span className="text-blue-600">{t(`client_dashboard.status_${order.status.toLowerCase()}`)}</span>
                  </p>
                  {order.scheduledAt && (
                    <p className="text-gray-600">
                ...
                <div className="flex items-center gap-2">
                  <div>
                    ...
                    >
                      {t('delivery_dashboard.confirm_status')}
                    </button>
                  </div>
                  <button
                  ...
                  >
                    {expandedOrder === order.id ? t('delivery_dashboard.hide_details') : t('delivery_dashboard.show_details')}
                  </button>
                </div>
              </div>
              {expandedOrder === order.id && order.statusHistory && (
                <div className="mt-4 p-4 bg-gray-50 rounded-lg">
                  <h4 className="text-lg font-semibold text-gray-800 mb-2">{t('delivery_dashboard.order_items')}</h4>
                  ...
                  <h4 className="text-lg font-semibold text-gray-800 mb-2">{t('delivery_dashboard.status_history')}</h4>
